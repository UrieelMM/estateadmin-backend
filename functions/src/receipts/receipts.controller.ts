...
export const sendReceiptsByEmail = onRequest(
  ...
  return corsHandler(req, res, async () => {
    try {
      ...
      console.log(`Buscando documentos para: yearMonth=${yearMonth}, condominiumId=${condominiumId}, clientId=${clientId}`);
      console.log(`Se enviarán todos los comprobantes/recibos al email del administrador: ${email}`);
      ...
      console.log(`Se encontraron ${snapshot.size} documentos en la consulta inicial`);
      ...
      for (const doc of snapshot.docs) {
        ...
        if (paymentGroupId && processedPaymentGroupIds.has(paymentGroupId)) {
          console.log(`Omitiendo documento ${doc.id} porque pertenece al grupo de pagos ${paymentGroupId} que ya fue procesado`);
          continue;
        }
        ...
        if (paymentGroupId) {
          ...
          console.log(`Marcando grupo de pagos ${paymentGroupId} como procesado`);
        }
        ...
        if (docType === 'recibos') {
          ...
          if (processedFilePaths.has(filePath)) {
            console.log(`Omitiendo archivo ${filePath} porque ya fue procesado anteriormente`);
            continue;
          }
          ...
          console.log(`Procesando comprobante: ${fileName}, ruta: ${filePath}`);
          ...
          try {
            ...
            try {
              ...
              console.log(`No se pudo hacer público el archivo ${filePath}, usando URL pública predeterminada:`, publicError);
            }
            ...
            console.error(`Error accediendo al archivo ${filePath}:`, storageError);
            continue;
          }
        }
        ...
        if (fileUrl) {
          try {
            console.log(`Descargando archivo desde: ${fileUrl.substring(0, 50)}...`);
            ...
            if (arrayBuffer.byteLength === 0) {
              console.error(`El archivo descargado para ${doc.id} está vacío`);
              continue;
            }
            
            const fileBuffer = Buffer.from(arrayBuffer);
            console.log(`Archivo descargado: ${fileName}, tamaño: ${fileBuffer.length} bytes`);
      ...
      if (archivosAgregados === 0) {
        ...
        res.status(404).send('No se encontraron archivos para enviar por correo.');
        return;
      }
